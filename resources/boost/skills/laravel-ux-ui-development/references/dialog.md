...
Header, Title, Description, and Overlay have no custom props. Every part accepts standard HTML attributes and Tailwind
classes.
...
Open the dialog initially with `<x-ux::dialog open>`. For client-owned state, bind the root with `x-model`. Use
`wire:model` only when the server must know whether the dialog is open, and back it with a boolean property.
...
- Use `:show-close-button="false"` only when another clearly labelled Close control is present or the workflow
  intentionally cannot be dismissed.
- Use `x-ux::dialog.close` for Cancel, Done, or custom close controls. A normal Button does not close the dialog
  automatically.
- For long content, constrain and scroll the body area rather than the entire Content panel:
  `-mx-4 no-scrollbar max-h-[50vh] overflow-y-auto px-4`.

## Accessibility and RTL

- Trigger renders a native `type="button"` by default and receives `aria-haspopup`, `aria-expanded`, and
  `aria-controls`.
...
- Do not place a second form inside a form solely because it appears in a dialog; teleporting changes visual placement,
